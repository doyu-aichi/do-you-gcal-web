<script lang="ts">
	const faqs = [
		...
		{
			question: '同友会サイト以外でも動作しますか？',
			answer: '動作しません。セキュリティ上、拡張機能の対象サイトはあいち同友会サイトに限定しています。同友会サイト以外のページでは、予定、組織、名簿キャッシュの読み取りや補助 UI の追加を行いません。'
		},
		{
		...
		},
		{
			...
			answer: '詳細画面から日時、場所、概要、出欠項目を読み直し、Googleカレンダー側のイベントを更新します。出欠返信は Chrome 拡張機能の保存領域に保存し、一覧やサイドパネルの状態補正に使います。'
		},
		{
		...
		},
		{
			...
			answer: '組織員一覧、名簿ページ、配信先ページを開いたときに検出した会員名簿を Chrome 拡張機能の保存領域に保存し、氏名、会社、所属バッジ、役職、メールを表示します。ページングと所属バッジによる絞り込みに対応しています。'
		},
		{
		...
		},
		{
			...
			answer: 'ページ内パネルは対象ページのリロードに合わせて再読み込みされます。同じブラウザセッション内では、最後に開いていたタブとパネルの開閉状態を Chrome 拡張機能のセッション保存領域に保存して復元します。Chrome サイドパネルはページとは別枠なので、マイスケジュールの月移動やページリロード後も表示を残しやすくなります。'
		},
		{
			question: 'Chrome サイドパネルには何が表示されますか？',
			answer: 'Googleカレンダー予定、マイスケジュールや行事活動一覧で検出したページ予定、組織図、名簿、カレンダー設定を表示します。組織図では切替と資料保管庫への移動もできます。対象サイト以外のタブでは、セキュリティ上、同友会サイト由来の情報は表示・更新しません。'
		},
		{
			question: '対象サイトの localStorage にキャッシュは残りますか？',
			answer: '現在のバージョンでは、Do You Gcal の永続キャッシュは対象サイト側の localStorage ではなく chrome.storage.local に保存します。旧バージョンの dyg-* キャッシュが残っている場合は、対象サイトを開いたときに拡張機能の保存領域へ移行し、移行できたものから対象サイト側のコピーを削除します。'
		},
		{
		...
		{
			question: 'ページ検出予定がサイドパネルに出ない場合は？',
			answer: 'マイスケジュールまたは行事活動一覧をアクティブなタブで開き、表示月を再取得してください。予定は表示されているページから検出するため、まだ開いていない月や詳細未確認の出欠状態は自動では確定しません。'
		}
	];
</script>
