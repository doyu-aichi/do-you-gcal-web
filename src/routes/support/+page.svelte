<script lang="ts">
	...
	const changelog = [
		['0.8.0', 'Chrome サイドパネル対応を強化し、ページリロード後もGoogleカレンダー予定、ページ検出予定、組織図、名簿、設定を確認できるようにしました。検出イベントの一覧表示、組織切替、資料保管庫リンクもサイドパネル側に追加しました。'],
		['0.7.x', 'Svelte 5 版ページ内サイドパネル、初回セットアップ、状態保持、Googleカレンダー複数参照、年度内月選択、終了済み予定トグル、組織図、名簿タブ、告知メール、出欠返信キャッシュ、申込時同期、案内メール/保管庫リンク、拡張機能保存領域へのキャッシュ移行を追加'],
	...
	const usageSections = [
		{
			...
			items: [
				...
				'よく使う場合は、Do You Gcal の拡張アイコンを Chrome ツールバーに固定します。',
				'あいどるへログインし、右上の Do You Gcal 耳タブ、またはツールバーの拡張アイコンから Chrome サイドパネルを開きます。',
		...
		},
		{
			...
			items: [
				...
				'カレンダータブまたは Chrome サイドパネルで、ページ検出予定とGoogleカレンダー予定を確認します。',
		...
		},
		{
			title: '組織図と名簿を使う',
			items: [
				'ページ内パネルまたは Chrome サイドパネルの組織図タブで、現在の所属、下位グループ、切替、資料保管庫への導線を確認します。',
	...
	<div class="grid two blocks">
		<section class="panel block">
			...
			<ul>
				...
				<li>よく使う場合は、Do You Gcal の拡張アイコンを Chrome ツールバーに固定してください。Chrome サイドパネルを使うと、ページリロード後も補助画面を残しやすくなります。</li>
				<li>初回はマイページを開いて、現在の組織名と組織メニューを検出させます。</li>
				<li>組織管理の組織員一覧を開いて、名簿キャッシュを作成します。</li>
				<li>名簿ページ上の「この組織を名簿キャッシュ更新」でも、表示中組織の名簿を更新できます。</li>
				<li>予定はマイスケジュールまたは行事活動一覧を開いて、表示月ごとに検出します。Chrome サイドパネルにもページ検出予定の一覧が表示されます。表示月はサイトで選択中の年度内から選びます。</li>
